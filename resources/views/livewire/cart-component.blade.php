...
<div class="container">
    ...
    @if(Cart::instance('cart')->count() >0)
        ...
        @if(Session::has('success_message'))
            ...
            <ul class="products-cart">
                ...
                <li class="pr-cart-item">
                    <div class="product-image">
                        <figure><img src="{{asset('assets/images/shoes')}}/{{$item->model->image}}" alt="{{$item->model->name}}"></figure>
                    </div>
                    <div class="product-name">
                        <a class="link-to-product" href="{{route('product.details',['slug'=>$item->model->slug])}}">{{$item->model->name}}</a>
                    </div>
                    @if($item->model->sale_price>0 && $sale->status==1 && $sale->sale_date > Carbon\Carbon::now())
							<div class="price-field produtc-price">Sale Price<p class="price"style="color:red; font-size:15px;padding-left:5px;">$.{{$item->model->sale_price}}</p>
						</div>
                        <div class="price-field produtc-price">Regular Price<p class="price"style="color:black; font-size:15px;padding-left:5px; text-decoration:line-through;">$.{{$item->model->regular_price}}</p>
						</div>
							@else
                    ...
                    <div class="quantity">
                        ...
                        <p class="text-center"><a href="" class="btn btn-primary" wire:click.prevent="switchToSaveForLater('{{$item->rowId}}')">Save For Later</a></p>
            ...
            @else
            <p class="alert alert-warning text-success" role="alert">No Product you have yet in your cart</p>
            @endif
        </div>
        ...
        <div class="order-summary">
						...
						<p class="summary-info"><span class="title">Discount ({{Session::get('coupon')['code']}}) <a href="#" wire:click.prevent="removeCoupon"><i class="fa fa-times text-danger fa-2x"></i></a> </span><b class="index"> -Rs.{{number_format($discount,2)}}</b></p>
						...
						<p class="summary-info"><span class="title">Tax ({{config('cart.tax')}}%)</span><b class="index">Rs.{{number_format($taxAfetrDiscount,2)}}</b></p>
        ...
        @else
				...
				<p>Add item in it now</p>
			         <a href="/shop" class="btn btn-success">Shop Now</a></div>
		@endif
        <div class="wrap-iten-in-cart">
					<h3 class="title-box" style="border-bottom:solid 1px; padding-bottom:15px;">{{Cart::instance('saveForLater')->count()}} Items(s) Save For Later</h3>
					@if(Session::has('success_message'))
						<div class="alert alert-success">
							<strong>Success</strong> {{Session::get('success_message')}}
						</div>
					...
					<ul class="products-cart">
						...
						<li class="pr-cart-item">
							...
							<p class="price" style="color:green;">{{$item->regular_price}}</p>
							
							</div>
							<div class="quantity">
							
								<p class="text-center"><a href="#" class="btn btn-primary" wire:click.prevent="moveToCart('{{$item->rowId}}')">Move To Cart</a></p>
							</div>
							<div class="delete">
								<a href="#" wire:click.prevent="removeFromSave('{{$item->rowId}}')" class="btn btn-delete" title="">
									<span>Delete from your Save To Later</span>
									<i class="fa fa-times-circle" aria-hidden="true"></i>
								</a>
							</div>
						</li> 
